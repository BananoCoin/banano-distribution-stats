...
const run = async () => {
  ...
  if (process.argv.length < 4) {
    ...
    knownAccountTypeMap.set('ban_1boompow14irck1yauquqypt7afqrh8b6bbu5r93pc6hgbqs7z6o99frcuym', 'source');
    knownAccountTypeMap.set('ban_3fo1d1ng6mfqumfoojqby13nahaugqbe5n6n3trof4q8kg5amo9mribg4muo', 'source');
    ...
    for (const knownAccountType of knownAccountTypeList) {
      ...
      if (type != 'distributed-to-known') {
        await index.getDistributionOverTime(httpsRateLimit, historyChunkSize, timeChunkFn, knownAccountTypeMap,
            account, amountByTimeChunkAndSrcDestTypeMap, whalewatch,
            DEBUG, VERBOSE, knownAccountTypeNbr, knownAccountTypeList.length);
